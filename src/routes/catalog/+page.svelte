      ...
      <div class="flex items-center gap-6 text-sm">
        ...
        {#if searchQuery}
          <div class="flex items-center gap-2">
            ...
            <span class="text-red-500 font-semibold">{filteredBooks.length} results</span>
          </div>
        {/if}
      </div>
    </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
        {#each filteredBooks as book}
          <article class="group cursor-pointer">
            <!-- Book Cover -->
            <div class="relative aspect-[2/3] rounded-xl overflow-hidden bg-zinc-900 ring-1 ring-zinc-800 transition-all duration-300 group-hover:ring-red-500/50 group-hover:shadow-lg group-hover:shadow-red-500/10 group-hover:-translate-y-1">
              {#if book.coverUrl}
                <img 
                  src={book.coverUrl} 
                  alt={book.title} 
            ...
            <!-- Book Info -->
            <div class="mt-3 px-1">
              <h2 class="text-sm font-medium text-zinc-200 group-hover:text-red-400 transition-colors duration-200 line-clamp-2 leading-snug">
                {book.title}
              </h2>
